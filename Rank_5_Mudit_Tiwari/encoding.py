...
class DummyEncoding(Encoding):
    ...
    def create_encoding_dict(self, X, y):
        ...
        return {col: [k for k, v in X[col].astype('object').value_counts().to_dict().items() if v>self.threshold]
                for col in self.categorical_columns}

    def apply_encoding(self, X_in, encoding_dict):
        X = self.convert_input(X_in)
        for col, values in encoding_dict.items():
            for val in values:
...
class ThresholdEncoding(Encoding):
    ...
    def create_encoding_dict(self, X, y=None):
        ...
        for col in self.categorical_columns:
            ...
            encoding_dict.update({col: {k:k for k,v in vc.items() if v > self.threshold}})
