...
#
# def get_eval_metric(scoring_metric):
#     if scoring_metric == 'auc' or scoring_metric == 'AUC':
...
#     return 'f1', f1_score(y_true, y_hat), True
#
# def get_eval_metric(eval_metric):
...
class Estimator(object):
    ...
    def __init__(self, model, n_splits=5, random_state=100, shuffle=True, validation_scheme=FoldScheme.StratifiedKFold,
                 cv_group_col=None, n_jobs=-1, early_stopping_rounds=None, categorical_features_indices=None,
                 variance_penalty=0, verbose=100, eval_metric='auc', scoring_metric=roc_auc_score, over_sampling=None,
                 **kwargs):
    ...
    def fit(self, x, y, use_oof=False, n_jobs=-1):
        ...
        fitted_models = []
        if use_oof:
            folds = CustomFolds(num_folds=self.n_splits, shuffle=self.shuffle, validation_scheme=self.validation_scheme,
            ...
            for i, (train_index, test_index) in enumerate(self.indices):
                ...
                if self.over_sampling is not None:
                ...
                if (isinstance(model, LGBMClassifier) and self.early_stopping_rounds is not None):
                    model.fit(X=x_train, y=y_train, eval_set=[(x_val, y_val)],
                        early_stopping_rounds=self.early_stopping_rounds, verbose=self.verbose, eval_metric=self.eval_metric)
            ...
            if isinstance(model, LGBMClassifier):
                if self.early_stopping_rounds is not None:
                    model.fit(X=x_train, y=y_train, eval_set=[(x_val,y_val)],
                        verbose=False, eval_metric=self.eval_metric, early_stopping_rounds=self.early_stopping_rounds)
